DECLARE SUB footer ()
'**************************************************************************
'*   title: Gauge Calibration version 6.10                                *
'*                                                                        *
'*                                                                        *
'*   last amended:24/03/11 (c) dcw 1990-2011                              *
'**************************************************************************
'**************************************************************************
'*   Constants                                                            *
'**************************************************************************
...
DECLARE SUB get160angle (angle160, mFormat$)
DECLARE SUB DishedEndDepth ()
...
MenuBegin:
	...
	fcount% = 1
	CLS
	ME$ = " Afriso Eurogauge Ltd                         Calibration System Version 6.10"
	CALL MainScreen
...
D0041:
	...
	SELECT CASE Choice%
		...
		CASE IS = 1
			gauge% = 1
			accuracy = 2   ' 2%
			CALL get160angle(angle160, mFormat$)
...
PRINT #1, ME$
PRINT #1, "Scale: "; RTRIM$(LTRIM$(scale$)); " ("; RTRIM$(LTRIM$(STR$(accuracy))); "% Accuracy +/- "; RTRIM$(LTRIM$(STR$(accuracyvolume))); " "; RTRIM$(LTRIM$(scale$)); ") Maximum Deviation"
...
PRINT #1, ""
PRINT #1, ""
PRINT #1, "Certificate valid until: "
PRINT #1, ""
PRINT #1, ""
PRINT #1, "Authorised by:  D C Ward "
PRINT #1, "                Technical Manager "

END SUB

DEFDBL Z
SUB get160angle (angle160, mFormat$)
'**************************************************************************
'*     Get160Angle                                                           *
'**************************************************************************
SHARED Choice%
...
aprint 6, 30, "ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿", 15
aprint 7, 30, "³  Scale Angle   ³", 15
aprint 8, 30, "ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ", 15
menu% = 5: row% = 9: col% = 28: menu$(0) = "12345"
menu$(1) = "1. 270 Degree Scale"
menu$(2) = "2. 240 Degree Scale"
menu$(3) = "3. 225 Degree Scale"
menu$(4) = "4. 210 Degree Scale"
menu$(5) = "5. 180 Degree Scale"
...
'**************************************************************************
SUB header
PRINT #1, "CERTIFICATE of GAUGE CALIBRATION ";
PRINT #1, "                Serial Number: "; wo$
PRINT #1, line$
PRINT #1, ""
PRINT #1, "ISSUED BY:    "
PRINT #1, "Afriso Eurogauge Ltd"
PRINT #1, "Unit 4, Satellite Business Village, Fleming Way, Crawley. RH10 9NE"
PRINT #1, "email: techoffice@eurogauge.co.uk   web: www.eurogauge.co.uk      "
PRINT #1, line$
PRINT #1, ""
PRINT #1, "SUPPLIED TO: "
PRINT #1, Cust$
...
PRINT #1, line$
PRINT #1, ""
PRINT #1, "Calibrated using: Calsys 6.10 Table Compiler, DialWrite Creator 2.0"
PRINT #1, "                  Druck DPI510 Pressure Calibrator Serial 1209-92"
PRINT #1, line$
PRINT #1, ""
PRINT #1, "CONFORMITY & ORIGIN:"
PRINT #1, "This is to certify that the gauge detailed on this certificate conforms to"
PRINT #1, "the requirements of the applicable purchase order and where appropriate"
PRINT #1, "to relevant United Kingdom and European Union standards."
PRINT #1, "Manufactured in the United Kingdom."
PRINT #1, line$
PRINT #1, ""
